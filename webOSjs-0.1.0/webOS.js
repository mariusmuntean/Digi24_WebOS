...
(function() {
	if (window.PalmSystem) {
		...
		} else if (webOS.device.platformVersionMajor
				&& webOS.device.platformVersionMinor) {
			try {
	...
	webOS.fetchAppInfo = function() {
		if (!this.appInfo) {
			...
			var paths = [
					...
					"file:///media/cryptofs/apps/usr/palm/applications/"
							+ appID + "/appinfo.json",
					"file:///usr/palm/applications/" + appID + "/appinfo.json" ];
			var index = paths[0].indexOf(appID);
			if (index > -1) {
				paths.unshift(paths[0].substring(0, index) + appID
						+ "/appinfo.json")
			}
...
(function() {
	var hasVKeyboard = !(webOS.platform.legacy
			&& webOS.device.platformVersionMajor && parseInt(webOS.device.platformVersionMajor) < 3);
	if (hasVKeyboard) {
		var state = {};
		Mojo = window.Mojo || {};
	...
	if ((webOS.platform.legacy || webOS.platform.open) && !window.cordova) {
		...
		window.Mojo.relaunch = function(e) {
			...
			if (lp["palm-command"] && lp["palm-command"] == "open-app-menu") {
				...
				fireDocumentEvent("webOSRelaunch", {
					type : "webOSRelaunch",
					detail : lp
				})
			}
	...
	webOS.notification = {
		showToast : function(params, callback) {
			...
			if (webOS.platform.legacy || webOS.platform.open) {
				...
				var id = PalmSystem.addBannerMessage(message, JSON
						.stringify(toastParams), icon, soundClass, soundFile,
						soundDurationMs);
				callback && callback(id)
			} else {
				if (message.length > 60) {
					console
							.warn("Toast notification message is longer than recommended. May not display as intended")
				}
				var reqParam = {
					sourceId : source,
					message : message,
				...
				this.showToastRequest = webOS.service.request(
						"palm://com.webos.notification", {
							method : "createToast",
							parameters : reqParam,
		...
		removeToast : function(toastId) {
			if (webOS.platform.legacy || webOS.platform.open) {
				...
				this.removeToastRequest = webOS.service.request(
						"palm://com.webos.notification", {
							method : "closeToast",
							parameters : {
								toastId : toastId
		...
		showDashboard : function(url, html) {
			if (webOS.platform.legacy || webOS.platform.open) {
				var dash = window.open(url, "_blank",
				...
				console
						.warn("Dashboards are not supported on this version of webOS.")
			}
		}
	}
})();
	...
	var log = function(level, messageId, keyVals, freeText) {
		if (window.PalmSystem) {
			if (keyVals && !isObject(keyVals)) {
				...
				freeText = null;
				console
						.warn("webOSLog called with invalid format: keyVals must be an object")
			}
			...
			if (window.PalmSystem.PmLogString) {
				if (level == levelDebug) {
					...
					window.PalmSystem.PmLogString(level, messageId, keyVals,
							freeText)
				}
			} else {
				console
						.error("Unable to send log; PmLogString not found in this version of PalmSystem")
			}
		}
	};
...
(function() {
	...
	webOS.service = {
		...
		systemPrefix : webOS.platform.legacy || webOS.platform.open ? "com.palm"
				: "com.webos",
		protocol : webOS.platform.legacy && webOS.device.platformVersionMajor
				&& parseInt(webOS.device.platformVersionMajor) <= 1 ? "palm://"
				: "luna://"
	};
	navigator.service = {
